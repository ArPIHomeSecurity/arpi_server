...
class PowerAdapter:
    ...
    def __init__(self):
        logger.debug("Power sense (MCP3008 AD2 ch0) creating...")
        self._sense = None
        ...
        try:
            ...
            logger.error("Failed to init MCP3008 for power sense: %s", e)
            self._sense = None
    ...
    def source_type(self):
        ...
        if self._sense is None:
            logger.error("Power sense not initialized!")
            return SOURCE_BATTERY

        try:
            value = self._sense.value
        except Exception as e:
            logger.error("Error reading MCP3008 for power sense: %s", e)
            return SOURCE_BATTERY

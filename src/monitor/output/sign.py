    ...
    def run(self):
        ...
        while not self._stop_event.is_set():
            ...
            if (
                ...
                logger.debug("No duration, break loop after delay")
                break

            if duration > Output.ENDLESS_DURATION and start_time + delay + duration <= now:
                logger.debug("Duration expired, break loop")
                break
        ...
        output_state = default_state
        self._output_adapter.control_channel(channel, output_state)
        logger.debug("Output sign on channel '%s' turned off", OUTPUT_NAMES[channel])
        logger.debug("Output sign exited on channel '%s'", OUTPUT_NAMES[channel])

...
def upgrade():
    ...
    with op.batch_alter_table("alert_sensor", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("name", sa.String(length=16), nullable=False, server_default="")
        )
        batch_op.add_column(sa.Column("silent", sa.Boolean(), nullable=True, server_default=None))
        batch_op.add_column(
            sa.Column("monitor_period", sa.Integer(), nullable=True, server_default=None)
        )
        batch_op.add_column(
            sa.Column("monitor_threshold", sa.Integer(), nullable=True, server_default="100")
        )
    ...
    with op.batch_alter_table("sensor", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("silent_alert", sa.Boolean(), nullable=True, server_default="false")
        )
        batch_op.add_column(
            sa.Column("monitor_period", sa.Integer(), nullable=True, server_default=None)
        )
    ...
    connection.execute(
        sa.text("UPDATE sensor SET name = substr(description, 1, 16) WHERE name = ''")
    )
    connection.execute(
        sa.text("UPDATE alert_sensor SET name = substr(description, 1, 16) WHERE name = ''")
    )
    connection.execute(
        sa.text("UPDATE arm_sensor SET name = substr(description, 1, 16) WHERE name = ''")
    )
